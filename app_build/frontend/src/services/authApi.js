 ...
 * @returns {Promise<{exists: boolean, farmer_id?: string}>}
 */
export const checkPhoneExists = async (phone) => {
  try {
    const response = await apiClient.post(API_ENDPOINTS.checkPhone, { phone });
    return response;
  } catch (error) {
    console.error('Check Phone Error:', error);
    throw error.response?.data || error.data || { message: 'Failed to check phone' };
  }
};
